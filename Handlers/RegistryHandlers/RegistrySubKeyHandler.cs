...
namespace Wildlands_System_Scanner.Registry
{
    ...
    {
        public static string EnumerateSubKeys(RegistryKey key, ref int index)
        {
        ...
        public static string EnumSubKey(IntPtr hKey, int index)
        {
            int bufferSize = 256;
            var buffer = new char[bufferSize];
            int result;

            while ((result = Advapi32NativeMethods.RegEnumKeyEx(hKey, index, buffer, ref bufferSize, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero)) == NativeMethodConstants.ERROR_MORE_DATA)
            {
                bufferSize *= 2;
                buffer = new char[bufferSize];
            }

            if (result == NativeMethodConstants.ERROR_NO_MORE_ITEMS) return null;
            if (result != NativeMethodConstants.ERROR_SUCCESS) throw new InvalidOperationException("Failed to enumerate registry keys.");
        ...
        public static bool INVALIDSUBKEY(string fullKey, out string invalidName)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        // Call NtEnumerateKey
                        int status = NtdllNativeMethods.NtEnumerateKey(hKey, index, KeyInformationClass, buffer, bufferSize, out resultLength);

                        if (status == NativeMethodConstants.STATUS_NO_MORE_ENTRIES || status == NativeMethodConstants.STATUS_BUFFER_OVERFLOW)
                        {
                            // Handle resizing the buffer if needed
                            Marshal.FreeHGlobal(buffer);
                            buffer = Marshal.AllocHGlobal(resultLength);
                            status = NtdllNativeMethods.NtEnumerateKey(hKey, index, KeyInformationClass, buffer, resultLength, out _);
                        }
            ...
            {
                NtdllNativeMethods.NtClose(hKey);
            }
        }
        ...
        public static void INVALIDSUBKEYDEL(string mainKey, ref int result)
        {
            ...
            try
            {
                ...
                {
                    // Enumerate subkeys
                    int status = NtdllNativeMethods.NtEnumerateKey(hKey, index, 1, buffer, 1024, out resultLength);

                    if (status == NativeMethodConstants.STATUS_NO_MORE_ENTRIES)
                        break; // No more entries, exit the loop

                    if (status == NativeMethodConstants.STATUS_BUFFER_TOO_SMALL)
                    {
                        // If the buffer is too small, allocate a larger one
                        Marshal.FreeHGlobal(buffer);
                        buffer = Marshal.AllocHGlobal(resultLength);
                        status = NtdllNativeMethods.NtEnumerateKey(hKey, index, 1, buffer, resultLength, out resultLength);
                    }
                    ...
                    // Extract subkey name
                    // Assuming buffer is an IntPtr pointing to a Unicode string
                    string name = Marshal.PtrToStringUni(buffer);

                    // Pass the string to the ExtractRegistryKey method
                    string extractedKey = RegistryKeyHandler.ExtractRegistryKey(name);
        ...
        public static void INVALIDSUBKEYNAMEFIX(string fix, string hFixLog, string deletedMessage, string notDeletedMessage, string invalidKeyMessage, string notFoundMessage)
        {
            ...
            RegistryKeyHandler.CheckRegistryKeyPermissions(mainKey);
